    ...
    def __init__(
        ...
        **_: Any,
        ...
        if acp_args is not None:
            resolved_args = acp_args
        elif args is not None:
            resolved_args = args
        else:
            resolved_args = _resolve_args()
        self._acp_args = list(resolved_args)
    ...
    def _run_acp_session(
        ...
        *,
        ...
        try:
            ...
            if requested_model and require_model_config:
                ...
                try:
                    ...
                    if selection is not None:
                        ...
                        logger.warning(
                            "%s does not offer model %r; using the session default.",
                            self._provider_label,
                            requested_model,
                        )
                except Exception as exc:
                    logger.warning(
                        "%s model selection for %r failed; continuing with the "
                        "session default: %s",
                        self._provider_label,
                        requested_model,
                        exc,
